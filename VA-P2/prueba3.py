...
# Definir rangos de colores para la detección
...
if circles is not None:
    ...
    for circle in circles[0, :]:
        ...
        if cv2.countNonZero(combined_mask) > 105:
            edges = cv2.Canny(mask_white, 100, 150, apertureSize=3)
            ...
            if edges.size > 0:
                ...
                for cnt in contours:
                    ...
                    if area > 1:
                        ...
                        # Ajusta estos valores para la forma de flecha que buscas
                        if len(approx) >= 6 and len(approx) <= 6:
                            cv2.circle(canvas, (x, y), r, (0, 255, 0), 2)

cv2.imshow("Detected Triangles", canvas)
cv2.waitKey(0)
cv2.destroyAllWindows()
